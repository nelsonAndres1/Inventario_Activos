...
@Component({ 
    selector: 'app-traslado-grupal', 
    templateUrl: './traslado-grupal.component.html', 
    styleUrls: ['./traslado-grupal.component.css'], 
    providers: [Conta19Service, ReporteService, Gener02Service, TrasladoService, Conta148Service] })
export class TrasladoGrupalComponent implements OnInit, AfterViewInit {
              ...
              if (!banderaCS) {
                if (this.activos.length > 0) {
                  for (let index = 0; index < this.activos.length; index++) {
                    console.log("entro3!");
                    if (this.activos[index].codact.trim() == id) {
                      console.log("entro4!");
    ...
    onSubmit() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                for (let index1 = 0; index1 < this.activos_I.length; index1++) {
                    if (this.lista_observacion.length > 0) {
                        for (let index2 = 0; index2 < this.lista_observacion.length; index2++) {

                            if (this.activos_I[index1].codact == this.lista_observacion[index2][1]) {
                                this.activos_I[index1].observacion = this.lista_observacion[index2][0];
                                bandera = true;
                            } else {
                                bandera = false;
                            }
                        ...
                        if (bandera == false) {
                            this.activos_I[index1].observacion = 'sin observacion';
                        }
                    } else {
                        this.activos_I[index1].observacion = 'sin observacion';
                    }
                ...
                for (let index1 = 0; index1 < this.activos_I.length; index1++) {
                    if (this.lista_estado.length > 0) {
                        for (let index2 = 0; index2 < this.lista_estado.length; index2++) {
                            if (this.activos_I[index1].codact == this.lista_estado[index2][1]) {
                                this.activos_I[index1].estado = this.lista_estado[index2][0];
                                bandera2 = true;
                            } else {
                                bandera2 = false;
                            }
                        ...
                        if (bandera2 == false) {
                            this.activos_I[index1].estado = 'B';
                        }
                    } else {
                        this.activos_I[index1].estado = 'B';
                    }
                ...
                for (let index = 0; index < this.activos_I.length; index++) {
                    this.conta19 = new Conta19(this.activos_I[index].codact, this.activos_I[index].subcod, this.activos_I[index].codbar, this.activos_I[index].estado, this.activos_I[index].detalle, this.activos_I[index].codare, this.activos_I[index].coddep, this.activos_I[index].codubi, this.activos_I[index].cedtra, this.activos_I[index].codcen, this.usuario, this.activos_I[index], this.activos_I[index].observacion);
                        ...
                        if (this.datoCe != this.cedtraConsultado.cedtra) {
                            ...
                            if (this.observacion_traslado.length > 0) {
                                ...
                                if (this.ubi_traslado > 0 && this.ubi_traslado < 99) {
                                    for (let index = 0; index < this.activos_I.length; index++) {
                                        this._trasladoService.updateConta19(new Conta19(this.activos_I[index].codact, '', '', '', '', '', this.depdesdes, this.ubi_traslado, this.datoCe, '', this.identity.sub, '', this.observacion_traslado)).subscribe(
                                            response => {
                                                if (response.status != 'error') {
                                                    this._conta19Service.consultConta65(new Conta65(this.activos_I[index].codact, this.cedtra)).subscribe(
                                                        response => {
                                                            this._trasladoService.saveConta65(new Conta65_copia('01', this.documento, this.identity.sub, this.activos_I[index].codact, response.subcod, this.observacion_traslado, response.areori, response.depori, response.ubiori, response.cedori, aredes, coddeoV, this.ubi_traslado, this.datoCe, 'A')).subscribe(
                                                                response => {
                                                                    console.log(this.activos_I);
                                                                    if (response.status = "success") {
                                                                        Swal.fire('Traslado Realizado!', '', 'success');
                                                                        if (!banderu_reporte) {
                                                                            this.generateInvoice(this.activos_I);
                                                                        }
